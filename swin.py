...
def get_device():
    if torch.cuda.is_available():
        ...
        if cap[0] >= 7:
            ...
            print("GPU not supported. Switching to CPU")
...
class MultiTaskDataset(Dataset):
    ...
    def __getitem__(self, idx):
        path, f_lbl, q_lbl = self.samples[idx]
        img = Image.open(path).convert("RGB")

        if self.transform:
            img = self.transform(img)

        return img, f_lbl, q_lbl
...
def evaluate(loader):
    ...
    fruit_acc = accuracy_score(f_true, f_pred)
    quality_acc = accuracy_score(q_true, q_pred)

    return fruit_acc, quality_acc, fruit_f1, quality_f1
...
for epoch in range(EPOCHS_STAGE1):
    ...
    f_acc, q_acc, f_f1, q_f1 = evaluate(val_loader)

    print(f"Epoch {epoch+1} | Loss {loss:.4f}")
    print(f"Fruit Acc {f_acc:.3f} | Fruit F1 {f_f1:.3f}")
    print(f"Quality Acc {q_acc:.3f} | Quality F1 {q_f1:.3f}\n")
...
f_acc, q_acc, f_f1, q_f1 = evaluate(test_loader)

print(f"Final Fruit Accuracy : {f_acc:.3f}")
print(f"Final Fruit F1       : {f_f1:.3f}")

print(f"Final Quality Accuracy : {q_acc:.3f}")
print(f"Final Quality F1       : {q_f1:.3f}")

torch.save(model.state_dict(), "swin_multitask.pth")

print("Swin Transformer multitask model saved")